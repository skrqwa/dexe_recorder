...
GstRecorder::~GstRecorder() { DestroyAll(); }
...
bool GstRecorder::CreateHeadPipeline(const std::string& session_dir, Format format) {
  ...
  if (format == Format::VIDEO) {
    ...
    encode_branch = "! nvvidconv compute-hw=1 "
  ...
  std::string pipe_str =
      ...
      "! videocrop left=0 right=" + std::to_string(half_w) + " top=0 bottom=0 "
      "! video/x-raw,format=I420,width=" + std::to_string(half_w) + ",height=" + std::to_string(input_height) + " "
      "! queue max-size-buffers=8 max-size-bytes=0 max-size-time=0 "
      + encode_branch + sink_left + " "
      // 右目：videocrop 裁掉左半
      "t. ! queue max-size-buffers=8 max-size-bytes=0 max-size-time=0 "
      "! videocrop left=" + std::to_string(half_w) + " right=0 top=0 bottom=0 "
      "! video/x-raw,format=I420,width=" + std::to_string(half_w) + ",height=" + std::to_string(input_height) + " "
      "! queue max-size-buffers=8 max-size-bytes=0 max-size-time=0 "
...
bool GstRecorder::CreateHandPipeline(const std::string& session_dir,
                                      const std::string& camera_name, Format format) {
  std::string path;
  ...
  if (format == Format::VIDEO) {
    encode_branch = "! nvv4l2h264enc bitrate=4000000 control-rate=0 peak-bitrate=8000000 "
                    "idrinterval=30 iframeinterval=30 insert-sps-pps=true "
                    "profile=0 num-B-Frames=0 "
                    "! h264parse ! qtmux ";
  ...
  if (format == Format::VIDEO) {
    pipe_str =
        "appsrc name=" + appsrc_name + " format=time do-timestamp=true is-live=true "
...
void GstRecorder::PushHandFrame(const std::string& camera_name, const uint8_t* data,
