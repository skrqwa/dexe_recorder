 ...
 * @brief dexe_recorder 主节点实现
 *
 * 订阅关节状态、相机、EE 话题，只更新内存快照。
 * 两个 30Hz 写线程取快照组装帧，流式写 pose_record.json（命令）和
...
RecorderNode::RecorderNode(const rclcpp::NodeOptions& options) : rclcpp::Node("dexe_recorder", options)
{
    ...
    if (cfg_path.empty())
    {
        ...
        std::string home_cfg = std::string(std::getenv("HOME")) + "/workspace/dexe_recorder/config/auto_recorder.yaml";
    ...
    if (!cfg_path.empty())
    {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Loaded config from %s (save_format=%s)",
                    cfg_path.c_str(),
    ...
    {
        RCLCPP_INFO(
            this->get_logger(), "No config loaded, using defaults (save_format=%s)", config_.save_format.c_str());
    }
    ...
    // 服务
    start_srv_ = this->create_service<std_srvs::srv::Trigger>("~/start_recording",
                                                              std::bind(&RecorderNode::HandleStart, this, _1, _2));
    stop_srv_ = this->create_service<std_srvs::srv::Trigger>("~/stop_recording",
                                                             std::bind(&RecorderNode::HandleStop, this, _1, _2));
    status_srv_ = this->create_service<std_srvs::srv::Trigger>("~/get_status",
                                                               std::bind(&RecorderNode::HandleStatus, this, _1, _2));
...
bool RecorderNode::StartRecording()
{
    ...
    RCLCPP_INFO(this->get_logger(),
                "Recording STARTED: session=%s, dir=%s",
...
std::string RecorderNode::ImagePath(const std::string& camera_type, uint64_t frame_id) const
{
    ...
    std::string dir;
    if (camera_type == "head_left")
        dir = "head/left";
    else if (camera_type == "head_right")
        dir = "head/right";
    else if (camera_type == "hand_left")
        dir = "hand/left";
    else if (camera_type == "hand_right")
        dir = "hand/right";
    else
        dir = "other";
